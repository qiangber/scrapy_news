...
class Stnn(IncrementCrawlSpider):
    name = "stnn"
    website = "星球环岛网"
    ...
    def get_news(self, response):
        try:
            ...
            loader.add_value('date', response.xpath('//span[@class="date"]/text()').extract_first())
            loader.add_value('date', response.xpath('//span[@class="post-time"]/text()').extract_first())
            loader.replace_value('date', ''.join(loader.get_collected_values('date')).strip() + ':00')
            ...
            loader.add_value('content', ''.join(response.xpath('//p[@class="summary"]/descendant::text()').extract()))
